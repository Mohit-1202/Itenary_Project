import { calculateMetrics, calculateScore, generateRationale } from "../utils/calculateMetrics";
        ...
        <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow">
          ...
          <tbody>
            {itineraries.map((trip, index) => {
              const { totalCost, totalDuration, totalActivities } = calculateMetrics(trip);
              const score = calculateScore(trip, itineraries);
              const rationale = generateRationale(trip, itineraries);

              return (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="py-2 px-4 border font-semibold">{trip.tripName || "Unnamed Trip"}</td>
                  ...
                  <td className="py-2 px-4 border font-bold text-blue-600">{score}</td>
                  <td className="py-2 px-4 border text-gray-700">{rationale}</td>
                </tr>
              );
            })}
          </tbody>
